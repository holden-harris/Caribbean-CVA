...
This analysis follows the same approach used in prior NOAA CVAs, applying the same FCVA logic model to a subset of biological sensitivity attributes (BSAs) scored in Module 4. Bootstrap uncertainty is quantified using the same draw-pile resampling method as Module 8.
...
> To use only the three movement-related attributes, remove `"Species range"` from `target_attributes` at the top of Scripts 1 and 2. No other code changes are required.
