...
|--------|---------|------------|-----------|
| `uncertainty-analyses.R` | Validate baseline score reproduction; run bootstrap resampling and leave-one-out influence analyses; produce final bootstrap summary table | `outputs/final-tallies-long/`, `outputs/{run_label}/final-scores-compiled/` | `outputs/{run_label}/analyses/uncertainty-loo/` |
| `2-make-uncertainty-figures.R` | Produce LOO influence bar chart and bootstrap uncertainty stacked bar chart | `outputs/{run_label}/analyses/uncertainty-loo/final-tables/`, `outputs/{run_label}/final-scores-compiled/` | `figures/{run_label}/` |
...
Both analyses use the same logic model implemented as `fcva_logic_model()`. The sensitivity (or exposure) component rank is assigned based on how many attribute means meet each threshold, controlled by the `rank_threshold` setting at the top of `uncertainty-analyses.R`:
...
The `rank_threshold` option is set using one of the named constants defined in the Analysis settings section:

| Constant | rank_threshold value | Very High threshold | High / Moderate threshold |
|----------|---------------------|--------------------|-----------------------------|
| `attr_means_current` | 1 | > 2 attribute means ≥ 3.5 (i.e., ≥ 3) | > 1 attribute means ≥ 3.0 / 2.5 (i.e., ≥ 2) |
| `attr_means_plus1` | 2 | > 3 attribute means ≥ 3.5 (i.e., ≥ 4) | > 2 attribute means ≥ 3.0 / 2.5 (i.e., ≥ 3) |

**The active run's `rank_threshold` and `sens_attrs_drop` are sourced from `config.R` via `active_run`.** For `broadened_distribution` the setting is `attr_means_plus1` (`rank_threshold = 2L`, all 14 sensitivity attributes); for `cross_region_comparable` it is `attr_means_current` (`rank_threshold = 1L`, 12 attributes — drops Genetic diversity and Predation and competition dynamics). Step 3 of `uncertainty-analyses.R` validates this agreement by reproducing all 25 baseline ranks from scratch and halting if any disagree.
...
outputs/{run_label}/analyses/uncertainty-loo/   # All analysis outputs (created by Script 1)
...
└── final-tables/                              # Analysis outputs used for figures
    ...
    └── table_bootstrap_final_summary.csv

figures/{run_label}/                           # Figures produced by Script 2
...
|---------|---------|-------------|
| `rank_threshold` | sourced from `config.R` via `active_run` | FCVA logic model threshold — controls how many attributes must meet each score cutoff to assign a component rank. `broadened_distribution` = 2L; `cross_region_comparable` = 1L. See [FCVA logic model](#fcva-logic-model) above. |
...
|----------|-----------|---------|-----------|
| `fcva_logic_model(mean_scores, rank_threshold)` | Numeric vector of attribute mean scores (1–4); integer threshold controlling how many attributes must meet each cutoff | 1-row tibble: `component_rank` (character), `component_score_numeric` (integer) | Steps 3, 4, 6, 7 |
...
| `outputs/{run_label}/final-scores-compiled/overall-vulnerability-rankings/attribute_means_uscar.csv` | Compiled mean scores per stock × attribute for both Sensitivity and Exposure; filtered by `attribute_type` and `score_type` in Step 1 | Steps 1, 3, 6, 7, 8 |
| `outputs/{run_label}/final-scores-compiled/overall-vulnerability-rankings/overall_vulnerability_scores_uscar.csv` | Finalized baseline vulnerability scores and ranks for all 25 stocks | Steps 1, 3, 4, 6, 7 |
| `data/exposure-factor-filter-long.csv` | Expert rubric filter: one row per stock × exposure factor with `include` (TRUE/FALSE) indicating which factors apply per stock. Generated by `4-final-attribute-exposure-scoring/3-calculate-overall-vulnerability-scores.R` and must exist before running this script. | Step 1 |
...
Attribute means are split into `sens_means_std` (14 qualitative Sensitivity attributes, filtered by `attribute_type == "Sensitivity"` and `score_type == "Qualitative"`) and `exp_means_std` (Exposure factors, filtered by `attribute_type == "Exposure"` and `score_type == "Calculated"`, then further filtered by `semi_join` to the expert-approved subset for each stock using `data/exposure-factor-filter-long.csv`). All 14 sensitivity attributes apply to all stocks; the number of exposure factors retained in `exp_means_std` varies by stock.
...
Same structure as Step 6 but for exposure factors. Each stock is tested only against its expert-approved subset of exposure factors (from `data/exposure-factor-filter-long.csv`), so the total number of combinations varies by stock and is less than 25 × 15:
...
Joins the bootstrap vulnerability rank distribution (`boot_stock_summary`) with the finalized baseline scores (`vuln_std`) into a single wide-format CSV — one row per stock. Vulnerability rank proportions are rounded to 2 decimal places. Stocks whose dominant bootstrap rank proportion fell below 0.75 (`borderline = TRUE`) receive a `†` suffix on their stock name. Rows are sorted by baseline vulnerability rank descending (Very High → High → Moderate → Low), then alphabetically within each rank group.

## Output files

### Intermediate (validation and QA) — `outputs/{run_label}/analyses/uncertainty-loo/intermediate/`
...
### Final analysis tables — `outputs/{run_label}/analyses/uncertainty-loo/final-tables/`
...
| `table_leave_one_out_exposure_long.csv` | Varies (one row per stock × expert-approved factor; total < 25 × 15) | `stock_name`, `factor_omitted`, `baseline_exp_rank`, `new_exp_rank`, `baseline_vuln_rank`, `new_vuln_rank`, `baseline_vuln_score`, `new_vuln_score`, `rank_changed`, `rank_change_direction` |
| `table_leave_one_out_exposure_summary.csv` | Varies (one row per factor present in any stock's approved subset; `n_stocks_tested` varies by factor) | `factor_omitted`, `n_stocks_tested`, `n_rank_changed`, `n_rank_lower`, `n_rank_higher`, `prop_rank_changed` |
| `table_sensitivity_attribute_scores_for_plot.csv` | 350 | `stock_name`, `attribute_name`, `mean_score`, `component_rank_baseline` |
| `table_exposure_factor_scores_for_plot.csv` | Varies (one row per stock × expert-approved exposure factor) | `stock_name`, `exposure_factor`, `mean_score`, `component_rank_baseline` |
| `table_bootstrap_final_summary.csv` | 25 | `Stock`, `N Exp factors`, `Exp`, `Sens`, `Vul`, `L`, `M`, `H`, `VH`; one row per stock with abbreviated baseline ranks, number of expert-approved exposure factors, and bootstrap proportion in each vulnerability rank (2 dp); borderline stocks flagged with † in `Stock` |
...
| File | Used by |
|------|---------|
| `outputs/{run_label}/analyses/uncertainty-loo/final-tables/table_leave_one_out_exposure_summary.csv` | Figure 1, Panel A |
| `outputs/{run_label}/analyses/uncertainty-loo/final-tables/table_leave_one_out_sensitivity_summary.csv` | Figure 1, Panel B |
| `outputs/{run_label}/analyses/uncertainty-loo/final-tables/table_bootstrap_uncertainty_stock.csv` | Figure 2, Panel A |
| `outputs/{run_label}/analyses/uncertainty-loo/final-tables/table_directional_effect_bootstrap.csv` | Figure 2, Panel B |
| `outputs/{run_label}/final-scores-compiled/overall-vulnerability-rankings/overall_vulnerability_scores_uscar.csv` | Figure 2 (stock ordering) |

## Output figures — `figures/{run_label}/`
...
**Key results visible in this figure** reflect the current analysis run. Because the exposure LOO now uses a per-stock expert-approved factor subset (rather than all factors uniformly), influence patterns may differ from prior runs: factors that apply to fewer stocks will show lower `n_stocks_tested`, and factors that are broadly applicable will dominate the ranking. Sensitivity attribute results (Panel B) are unaffected by the exposure filter change.
...
**Key results visible in this figure** reflect the current analysis run. The bootstrap resamples only sensitivity tallies (exposure scores are held fixed at the updated per-stock filtered baseline), so bootstrap results will update when `uncertainty-analyses.R` is re-run following changes to the baseline produced by `3-calculate-overall-vulnerability-scores.R`. Directional effects (Panel B) are independent of the exposure factor filter and are unaffected by that change.
