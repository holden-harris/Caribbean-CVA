...
parse_scorer <- function(path){
  ...
  scorer <- substr(scorer, 1, 2) ## Change scorer name to initials
...
write.csv(
  species_reviews,
  file = file.path(out_dir, "n_reviews_final_scores.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(out_dir, "table_final_attribute_scores_all.csv"),
  row.names = FALSE
)
